...
/**
 * Começa a composição durante a **inicialização do ambiente**, em vez de esperar a primeira invocação.
 * Sem isso, a primeira credencial real pagaria o gerenciador de segredos e a importação do PEM.
 *
 * O handshake do banco fica **de fora**: o pool é preguiçoso e só conecta na primeira consulta. É proposital — abrir a conexão aqui faria um banco
 * indisponível derrubar a composição inteira, trocando o `503` correto por um `500` de inicialização.
 *
 * A rejeição é engolida de propósito:`getDependencies` já registra a causa e * descarta o cache, e a invocação seguinte reencontra o erro.
 */
